...
# CONFIG
# =========================
st.set_page_config(page_title="SURVEILLANCE Portfolio", layout="wide")
...
# =========================
# FUNZIONE INPUT STILIZZATA
# =========================
def styled_input(label, key, value=0.0):
    value = float(value)
    with st.container():
        st.markdown(
            f'<div class="input-container"><div class="cell-label">{label}</div>', unsafe_allow_html=True
        )
        val = st.number_input(
            "", value=value, key=key, label_visibility="collapsed", format="%.2f"
        )
        st.markdown('</div>', unsafe_allow_html=True)
    return val

# =========================
# TAB1: CALIBRA SURVEILLANCE
# =========================
with tab1:
...
with tab2:
    ...
    st.markdown("""
    <style>
    .input-container { background: linear-gradient(135deg, #fff9c4 0%, #fff176 100%); padding:10px; border-radius:5px; border:1px solid #fbc02d; height:100%; display:flex; flex-direction:column; justify-content:center;}
    ...
    with c1: start_pos = styled_input("START", "start_pos", 0.0)
    with c2: end_pos = styled_input("END", "end_pos", 0.0)
    with c3: incr_display_pos = st.empty()
    with c4: var_display_pos = st.empty()

    c5, c6, c7, c8 = st.columns(4)
    with c5: qty_pos = styled_input("QTY", "qty_pos", 0.0)
    ...
    with c12: out_f_input = styled_input("OUT/F", "out_f", 0.0)
    with c13: cst_display = st.empty()
    with c14: gr_inc_display = st.empty()
    with c15: gr_pl_display = st.empty()
    with c16: atx_input = styled_input("ATX%", "atx", 0.0)
    ...
    with n1: start_neg = styled_input("START", "start_neg", 0.0)
    with n2: end_neg = styled_input("END", "end_neg", 0.0)
    with n3: incr_display_neg = st.empty()
    with n4: var_display_neg = st.empty()

    n5, n6, n7, n8 = st.columns(4)
    with n5: qty_neg = styled_input("QTY", "qty_neg", 0.0)
    ...
    calculate = col_btn.button("CALCOLA/RESET", use_container_width=True, type="primary")
    ...
    if calculate:
        ...
        with c9: styled_output("OUT", val_out_pos, is_out=True)
        ...
        val_var_neg = (val_incr_neg / start_neg * 100) if start_neg !=0 else 0.0
