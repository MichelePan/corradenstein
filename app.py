...
def load_data_multiple(tickers_list):
    """Scarica tutti i ticker in una volta sola, nascondendo output di yfinance."""
    tickers = list(tickers_list)  # lista semplice e hashable
    buffer = io.StringIO()
    with contextlib.redirect_stdout(buffer):
        df = yf.download(tickers=tickers, period="5y", interval="1d", progress=False, group_by='ticker')
    return df
...
with tab1:
    ...
    # Barra di avanzamento creata sempre
